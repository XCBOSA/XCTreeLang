...
internal class XCTLMorethanStatement : XCTLStatement {
    
    var type: XCTLStatementType { .typeMorethan }
    ...
    func matchSelfStatement(lex: XCTLLexer) throws -> XCTLStatement? {
        if try lex.next().type == .typeMorethan {
            return XCTLMorethanStatement()
        }
        return nil
    }
